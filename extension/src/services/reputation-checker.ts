...
export async function checkReputation(
  ...
  // Try backend for real reputation data
  if (backendUrl) {
    try {
...
function heuristicReputationCheck(
  ...
  try {
    ...
    // Known safe domains (major sites that always have good security)
    ...
    if (trustedDomains.includes(baseDomain) || trustedDomains.includes(hostname)) {
      // Trusted domain — strong safety signal
      risk -= 30;
    }

    // Suspicious path patterns (login pages on non-standard domains)
    const path = parsed.pathname + parsed.search;
    if (/login|signin|auth|verify|account|password|update|secure|banking/i.test(path)) {
      // Only flag if it's on a non-trusted domain
    ...
    // Long hostname (phishing often uses long random strings)
  ...
  // Determine status based on risk and signals
